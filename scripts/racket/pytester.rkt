...
(define SAVE-FILE-PATH (build-path (path-only (path->complete-path (find-system-path 'run-file))) ".save"))
...
(define files-from-command-line
  (command-line
   #:program "PyTester"
   #:once-any
   [("-a" "--add") "Add files for the tester to keep track of"
                   (mode 'add)]
   [("-d" "--delete") "Deletes specified files from the tester"
                      (mode 'remove)]
   [("-c" "--clear") "Removes all registred files from the tester"
                     (mode 'reset)]
   [("-l" "--list") "List all files that the tester is tracking"
                    (mode 'list)]
   [("-t" "--test") "Runs all registered tests. If args is supplied then just runs the given tests"
                   (mode 'test)]
   [("-p" "--test-postgres") "Runs all registered tests. If args is supplied then just runs the given tests"
                   (mode 'test-postgres)]
   #:args filepaths
   filepaths))
...
(define (run)
  ...
  (match (mode)
    ...
    ['list (for [(line (file->lines SAVE-FILE-PATH))]
             (displayln line))]
...
(define (add-files file-paths)
  ...
  (display-to-file
   (string-join (remove-duplicates (append files file-paths)) "\n")
...
(define (remove-files file-paths)
  ...
  (display-to-file
   (string-join (remove-all file-paths files) "\n")
   (path->string SAVE-FILE-PATH)
   #:mode 'text
   #:exists 'replace))
...
(define (run-tests files #:mode mode)
  ...
  (define (fmt-path path-string)
    (string-replace (path->string (path-replace-extension
  ...
  (for [(file files-to-run)]
    ...
    (system (format "tox -e ~a -- ~a" mode (fmt-path file))
            #:set-pwd? true)
    ))
